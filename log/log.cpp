...
bool Log::init(const char*file_name, int log_buf_size , int spilt_lines, int max_queue_size) {
    //如果设置了max_queue.size,则设置为异步
    if(max_queue_size >= 1) {
        m_is_async = true;
...
void Log::write_log(int level, const char *format, ...) {
    ...
    if(m_today != my_tm.tm_mday || m_count % m_spilt_lines == 0) //everyday log
    {
        char new_log[256] = {0};
        fflush(m_fp);
        fclose(m_fp);
